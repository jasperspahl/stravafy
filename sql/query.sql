...
-- name: GetLastHistoryEntryBefore :one
SELECT id,
       timestamp,
       is_playing,
       ctx.type          ctx_type,
       ...
       item.episode_show_uri
FROM spotify_user_history
         LEFT JOIN main.spotify_user_history_context ctx on spotify_user_history.id = ctx.history_id
         LEFT JOIN main.spotify_user_history_item item on spotify_user_history.id = item.history_id
WHERE user_id = ?
  AND timestamp <= ?
ORDER BY timestamp DESC
LIMIT 1;
